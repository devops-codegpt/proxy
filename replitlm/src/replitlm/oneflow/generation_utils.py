...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # synced_gpus: 是否继续运行 while 循环直到达到 max_length。
        # 除非被覆盖，否则在 DeepSpeed ZeRO Stage 3 多 GPU 环境中，为了避免一个 GPU 在其他 GPU 之前完成生成而挂起，
        # 此标志将被设置为 True。否则，它将被设置为 False。
        ...
        # stopping_criteria 用于后续判断何时停止解码。若设置了解码最大长度，则验证已获取的stopping_criteria是否设置正确。
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        # keep track of which sequences are already finished
        # 初始化 unfinished_sequences, 维度为[batch_size], 用于判断batch内句子是否已全部解码完成, 值为1表示未解码完成, 0表示
        # 已解码完成。
        unfinished_sequences = torch.ones(input_ids.shape[0], dtype=torch.long, device=input_ids.device)

        # 初始化this_peer_finished为False, 用于说明当前GPU并未完成batch内所有句子的解码。
        # 仅在synced_gpus为True时起作用。synced_gpus为是否需要进行gpu间同步的标志。
        this_peer_finished = False  # used by synced_gpus only
        ...
        while True:
            if synced_gpus:
                ...
                # The following logic allows an early break if all peers finished generating their sequence
                # 如果需要进行GPU间的同步
                # 首先初始化 this_peer_finished_flag, 若当前GPU已完成batch内的所有句子的解码，则赋值为0.0, 否则赋值为1.0.
                this_peer_finished_flag = torch.tensor(0.0 if this_peer_finished else 1.0).to(input_ids.device)
                # send 0.0 if we finished, 1.0 otherwise
                # this_peer_finished_flag 变量进行相加，若其值为0.0, 说明所有GPU都已完成解码，此时可以结束解码。
                dist.all_reduce(this_peer_finished_flag, op=dist.ReduceOp.SUM)
                # did all peers finish? the reduced sum will be 0.0 then
                if this_peer_finished_flag.item() == 0.0:
                    break
            ...
            # 如果需要进行GPU间的同步，且当前GPU已对batch内所有句子解码完成, 则跳过
            if synced_gpus and this_peer_finished:
                continue  # don't waste resources running the code we don't need

            # 获取next_token_logits, 维度为[batch_size, vocab_size], 即预测的下一个token的logits。
            next_token_logits = outputs.logits[:, -1, :]

            # pre-process distribution
            # logits_processor对next_token_logits进行预处理, logits_processor为LogitsProcessorList实例
        ...
        # 解码结束，返回结果
        # 若需要返回生成过程中的详细结果, 则根据架构为encoder_decoder和decoder-only分别返回对应的dict
        # 否则直接返回预测序列
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
